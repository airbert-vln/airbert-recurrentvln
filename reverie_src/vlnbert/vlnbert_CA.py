...
class BertObjectEmbeddings(nn.Module):
    """Construct the embeddings from image, spatial location (omit now) and token_type embeddings.
    """
    def __init__(self, config):
        super(BertObjectEmbeddings, self).__init__()
        self.config = config
    ...
    def forward(self, obj_feat, obj_pos, t):
        img_embeddings = self.image_embeddings(obj_feat[..., :-4])
        # Note: modified for vln >
        a = self.image_location_embeddings(obj_pos)
        b = self.image_orientation_embeddings(obj_feat[..., -4:])
        loc_embeddings =  a + b
        # Note: modified for vln <
        embeddings = self.LayerNorm(img_embeddings+loc_embeddings)
        embeddings = self.dropout(embeddings)
...
class VLNBert(BertPreTrainedModel):
    ...
    def forward(self, mode, input_ids, token_type_ids=None,
        ...
        if mode == 'language':
            ...
            state_visn_embeds = torch.cat([state_embeds.unsqueeze(1), cand_embeds, obj_embeds], dim=1)
            state_visn_masks = torch.cat([
                torch.ones(batch_size, 1, dtype=torch.bool, device=device),
                cand_masks, obj_masks
            ], dim=1)
            ...
            state_proj = self.v_pooler(self.dropout(state_output.unsqueeze(1)))
            ...
            visual_action_scores = torch.cat([visual_action_scores, stop_scores.unsqueeze(1)], dim=-1)
