...
class CaptionBertSelfAttention(BertSelfAttention):
    ...
    The visual mode will skip self-attention update for language instructions.
    """
    ...
    def forward(self, mode, hidden_states, attention_mask, head_mask=None,
        ...
        if mode == 'visual':
            mixed_query_layer = mixed_query_layer[:, [0]+list(range(-self.config.directions-self.config.obj_directions, 0)), :]
        ...
        # (batch, num_attn_heads, query_len, key_len)
...
class CaptionBertAttention(BertAttention):
    ...
    def forward(self, mode, input_tensor, attention_mask, head_mask=None,
        ...
        if mode == 'visual':
            attention_output = self.output(self_outputs[0], input_tensor[:, [0]+list(range(-self.config.directions-self.config.obj_directions, 0)), :])
        if mode == 'language':
...
class CaptionBertEncoder(BertEncoder):
    ...
    def forward(self, mode, hidden_states, attention_mask, head_mask=None,
        ...
        if mode == 'visual':
            # iterate over the 12 Bert layers (language features only as Keys and Values
            for i, layer_module in enumerate(self.layer):
                ...
                # cat language hidden states in the inputs again
                concat_layer_outputs = torch.cat((layer_outputs[0][:,0:1,:], 
                    hidden_states[:,1:-self.config.directions-self.config.obj_directions,:], 
                    layer_outputs[0][:,1:self.config.directions+self.config.obj_directions+1,:]), 1)
                hidden_states = concat_layer_outputs

                if i == self.config.num_hidden_layers - 1:
                    # (batch, num_attn_heads, query_len, key_len)
                    # output the attention score on images and objects
                    last_attention_score = layer_outputs[1][:, :, 0, :-self.config.obj_directions]
                    last_attention_score_obj = layer_outputs[1][:, :, 0, -self.config.obj_directions:]

            outputs = (hidden_states, last_attention_score, last_attention_score_obj)
...
class BertImgModel(BertPreTrainedModel):
    ...
    def forward(self, mode, input_ids, token_type_ids=None, attention_mask=None,
        ...
        # broadcast to (batch_size, num_attn_heads, query_len, key_len)
        ...
        # since attention_mask is 1 for positions we want to attend and 0 for masked position
        # this operation will create a tensor which is 0 for position we want to attend
        # and -10000.0 for masked positions. 
        # since we are adding the mask to the raw scores before the softmax.
...
class VLNBert(BertPreTrainedModel):
    ...
    def forward(self, mode, input_ids, token_type_ids=None, attention_mask=None,
        ...
        if mode == 'language':
        ...
        elif mode == 'visual':  # (4, 768), (8, 12, 10), 12 is the 12 heads in the last BERT layer
            attention_scores_obj = outputs[3].mean(dim=1)   # mean over 12 heads
            ...
            visual_attention_scores = torch.cat([visual_attentions.mean(dim=1), stop_scores.unsqueeze(1)], dim=-1)

            state_proj = pooled_output

            return state_proj, visual_attention_scores, attention_scores_obj
