...
class R2RBatch():
    ...
    def __init__(self, feature_store, obj_feats, batch_size=100, seed=10, splits=['train'], tokenizer=None,
                 name=None):
        ...
        for split in splits:
            for i_item, item in enumerate(load_datasets([split])):
                ...
                if args.test_only and i_item == 64:
                    break
                if "/" in split:    # useless here, no augmented data in REVERIE
                    try:
                        new_item = dict(item)
                ...
                for vis_pos, objects in self.obj_feats[state.scanId][state.location.viewpointId].items():
                    for objId, obj in objects.items():
                        ...
                        if args.vlnbert == 'vilbert':
                            # object feature only use 4 angle dim
                            obj_features.append(np.concatenate((obj['features'].toarray().squeeze(), directional_feature[int(vis_pos), :4]), -1))
