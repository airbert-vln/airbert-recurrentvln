...
def load_datasets(splits):
    ...
    for split in splits:
        ...
        if "/" not in split:
            with open('data/REVERIE/REVERIE_%s.json' % split) as f:
...
def loadObjProposals():
    bboxDir = 'data/REVERIE/BBox'
    objProposals = {}   # {scan_vp: {bbox: [(x, y, w, h)], visible_pos: [viewid], objId: [objid]}}
    obj2viewpoint = {}  # {scan_objid: viewpoint_list}
