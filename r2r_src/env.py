...
class EnvBatch():
    ...
    def getStates(self):
        ...
        :return: [ ((36, feature_size), sim_state) ] * batch_size
        """
        feature_states = []
...
class R2RBatch():
    ...
    def __init__(self, feature_store, batch_size=100, seed=10, splits=['train'], tokenizer=None,
        ...
        if tokenizer:
            self.tok = tokenizer
        scans = []
        ft_scans = set([x.split('_')[0] for x in feature_store.keys()])
        for split in splits:
            for i_item, item in enumerate(load_datasets([split])):
                if item['scan'] not in ft_scans:    # for fast testing codes
                    continue    
...
class R2RObjectBatch(R2RBatch):
    def __init__(self, feature_store, obj_feature_store, batch_size=100, seed=10, 
                 splits=['train'], tokenizer=None, name=None):
        super().__init__(feature_store, batch_size=batch_size, seed=seed,
            splits=splits, tokenizer=tokenizer, name=name)
        self.obj_feature_store = obj_feature_store
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            # object features
            obj_feats, obj_pos, obj_viewindexs = self.obj_feature_store[((state.scanId+'-'+state.location.viewpointId).encode(), state.heading)]
            if args.obj_in_cand_view:
                masks = np.zeros(len(obj_viewindexs), dtype=np.bool)
                for cand in candidate:
                    pointid = cand['pointId']
                    masks = masks | (obj_viewindexs == pointid)
                # shape can be 0
                obj_feats = obj_feats[masks]
                obj_pos = obj_pos[masks]
                # print(len(obj_feats), np.sum(masks))
            obs[-1].update({
                'obj_feats': obj_feats,
                'obj_pos': obj_pos,
            })
