        ...
        if mode == 'visual':
            for i, layer_module in enumerate(self.layer):
                ...
                concat_layer_outputs = torch.cat((layer_outputs[0][:,0:1,:], 
                    hidden_states[:,1:-self.config.directions,:], 
                    layer_outputs[0][:,1:self.config.directions+1,:]), 1)
...
class VLNBert(BertPreTrainedModel):
    ...
    def forward(self, mode, input_ids, token_type_ids=None, attention_mask=None,
        ...
        if mode == 'language':
            ...
            # the language_seq should be the same as input_ids except on the <CLS> state
            # why do not attend visual_seq but use the raw image features?
            ...
            return state_proj, visual_attention_scores, \
                   language_attention_probs.squeeze(-1), \
                   visual_attention_probs.squeeze(-1)
