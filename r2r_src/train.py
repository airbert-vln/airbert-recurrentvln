...
def train(train_env, tok, n_iters, log_every=2000, val_envs={}, aug_env=None):
    ...
    for idx in range(start_iter, start_iter+n_iters, log_every):
        ...
        # Run validation
        loss_str = "iter %d IL_loss %.2f RL_loss %.2f critic_loss %.2f entropy %.2f" % (iter,
            IL_loss, RL_loss, critic_loss, entropy)
...
def valid(train_env, tok, val_envs={}):
    ...
    record_file = open(os.path.join(log_dir, 'valid_log.txt'), 'a')
    record_file.write(str(args) + '\n\n')
    record_file.close()
    ...
    for env_name, (env, evaluator) in val_envs.items():
        ...
        detailed_results = agent.get_results(detailed=True)
        ...
        if args.submit:
            ...
            json.dump(
                detailed_results,
                open(os.path.join(log_dir, 'detailed_%s.json' % env_name), 'w')
            )
